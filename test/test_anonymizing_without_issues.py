...
class TestAnonymizingWithoutIssues(BaseTestClass):
    ...
    def test_01(self):
        ...
        self.base_dir_name = \
            'runs/' + self.create_dir_name_starting_with_datetime(
                [self.jira_version, self.get_system_default_languange()])

        pathlib.Path(self.base_dir_name).mkdir(parents=True)

        self.usernames_for_user_list_file = []

        self.expected_report_generator = ExpectedReportGenerator()
        self.expected_report_generator.jira_version = self.jira_serverinfo['versionNumbers']

        # This user might exist. In that case ignore the error.
        r = self.jira_admin_session.user_create('new_owner', 'new_owner@example.com', 'New Owner',
                                                password=self.STD_PASSWORD, notification=False)
        ...
        # User with upper- and lower-case chars in name.
        r = self.jira_admin_session.user_create('User1Post84', 'User1Post84@example.com', 'User 1 Post 84',
                                                password=self.STD_PASSWORD, notification=False)
        r.raise_for_status()
        ...
        # A user that will be kept an active user.
        r = self.jira_admin_session.user_create('User2Post84', 'User2Post84@example.com', 'User 2 Post 84',
                                                password=self.STD_PASSWORD, notification=False)
        r.raise_for_status()
        ...
        r = self.jira_admin_session.user_create('user5post84', 'user5post84@example.com', 'user5post84',
                                                password=self.STD_PASSWORD, notification=False)
        r.raise_for_status()
        ...
        # User with only lower-case letter in name; deleted.
        r = self.jira_admin_session.user_create('user9post84', 'user9post84@example.com', 'user9post84',
                                                password=self.STD_PASSWORD, notification=False)
        r.raise_for_status()
        ...
        # Jira doesn't anonymize such user-name. And in versions less than 8.10 the user-key neither.
        # But it anonymizes the user display-name.
        r = self.jira_admin_session.user_create('JIRAUSER21111', 'JIRAUSER21111@example.com', 'JIRAUSER21111',
                                                password=self.STD_PASSWORD, notification=False)
        r.raise_for_status()
        ...
        for user_name in self.usernames_for_user_list_file:
            ...
            if user_name in ['User2Pre84', 'User2Post84']:
                continue
            r = self.jira_admin_session.user_deactivate(user_name)
            r.raise_for_status()

        # Get predicted user-data (before some users will be deleted).
        r = self.get_predicted_anonymized_usernames(self.usernames_for_user_list_file)
        r.raise_for_status()
        ...
        with open(self.base_dir_name + '/predicted_anonymized_usernames.json', 'w') as f:
        ...
        if self.is_include_users_from_generated_test_resouces:
            r = self.jira_admin_session.user_remove('user9pre84')
            r.raise_for_status()
        r = self.jira_admin_session.user_remove('User9Post84')
        r.raise_for_status()
        ...
        # Get predicted anonymized user display-names.
        r = self.get_predicted_anonymized_usernames(self.usernames_for_user_list_file)
        r.raise_for_status()
    ...
    def create_issue_and_update_userpicker_customfield_by_user(self, user_name):
        ...
        r = self.jira_admin_session.update_issue_field(issue_key, {'reporter': {'name': 'admin'}})
        r.raise_for_status()
...
class ExpectedReportGenerator:
    jira_version: List[int] = field(default_factory=list)
    ...
    def _is_jiraversion_ge810(self):
        """Check if Jira-version is greater or equal than 8.10. """
        return self.jira_version[0] > 8 or self.jira_version[0] == 8 and self.jira_version[1] >= 10

    def _is_jiraversion_lt810(self):
        """Check if Jira-version is less than 8.10. """
        return self.jira_version[0] == 8 and self.jira_version[1] < 10

    def generate(self):
        is_jiraversion_lt810 = self._is_jiraversion_lt810()
        self.report = {'overview': self.overview, 'users': self.users}

        for user in self.users:
            paud_for_user = self.predicted_anonymized_userdata[user.user_name]

            if is_jiraversion_lt810 and user.deleted:
