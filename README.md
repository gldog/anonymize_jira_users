...
- [The "Features"-feature](#the--features--feature)
  * [Feature "do_report_anonymized_user_data"](#feature--do-report-anonymized-user-data-)
...
- Create a config-file-template: `anonymize_jira_users.py misc -g`.
...
- Call `anonymize_jira_users.py validate -c my-config.cfg` to see what would happen in case of 
  anonymizing.
...
The Anonymizer has the three commands `validate`, `anonymize`, and `misc`.
...
parameter-list). 
    ...
    -f FEATURES [FEATURES ...], --features FEATURES [FEATURES ...]
                          Features need external tools. See the docs. Choices:
                          do_report_anonymized_user_data
...
encoding. You can possibly fix this with an explicit character-encoding.
To get an idea what Python suggests on your current platform, execute 
...
If is set to `false`, the users will be anonymized. But with set to `true`, the 
Anonymizer _tries_ to delete users. If deletion failed for a specific user, this
user will be anonymized. The reports show what finally happened to each user.
...
The validation is a subset of the anonymization. So validation is done any time
the anonymization is done. With validation only you can get an impression what 
would happen in case of anonymization.
...
It seems obvious not-existant users cannot be anonymized. But in fact, anonymization 
is done by user-key, not user-name. And user-keys remain after deletion. So it is 
possible to anonymize deleted users. But the anonymizer doesn't do this. If you 
are interested anonymizing deleted users, have a look at the
[Jira Anonymization REST API](https://docs.atlassian.com/software/jira/docs/api/REST/8.13.2/#api/2/user/anonymization).
...
The durations of anonymization depend on the amount of issues, user-custom-fields,
and I think also on the DB-type. E.g. (in Jira 8.13.1):
...
- anonymizing_report.json: Information about the anonymized or just validated users in JSON format.
  It is generated from anonymizing_report_details.json. 
- anonymizing_report.csv: Content as anonymizing_report.json, but in comma-separated format. 
  The content is generated from and equal to the report in JSON format.
...
validation error reported by the anonymization REST-API. 
A validation error could be the fact a user is connected to a read-only directory.
...
The number of deleted and anonymized users are always 0, because `validate` only 
checks if the users could be anonymized.
...
The second user `user1post84` is also an inactive, local user, but was created 
in Jira 8.4 or later. Since Jira 8.4, the user-keys are something like 
JIRAUSER12345. The user-key of `user1post84` is `JIRAUSER10200`.

`anonymize_jira_users.py` does not decide between users created before or since
Jira 8.4. But just for the case you are curious why some user-keys in the 
Anonymizer's reports are equal to the user-names, and some not.
...
The output is:
    ...
    2020-12-27 21:27:55,652:INFO:read_user_names_from_infile():   The user-names are: ['user1pre84', 'user1post84']
    ...
    2020-12-27 21:27:55,779:INFO:filter_users(): Remaining users to be anonymized: ['user1pre84', 'user1post84']
    Anonymizing Result:
      Users in infile:   2
      Skipped users:     0
      Deleted users:     0
...
This means, the filter has left over both users for anonymization. In other words: Both users haven't matched to any
criteria not to anonymize a user. So thumps up for both users.

If the filter had found any criteria not to anonymize a
user, it would have given an `filter_error_message`.
...
Beside the `anonymization_report.json` the `anonymization_report.csv` has been 
created and looks like the following screenshot:
...
The output is:
    ...
    2020-12-27 21:34:39,777:INFO:read_user_names_from_infile():   The user-names are: ['user1pre84', 'deleted-user', 'user-from-ad']
    ...
    2020-12-27 21:34:39,835:INFO:filter_users(): Remaining users to be anonymized: []
    Anonymizing Result:
      Users in infile:   3
      Skipped users:     3
      Deleted users:     0
...
`GET /rest/api/2/user?username=user1pre84`. It returns HTTP status-code `200 OK`, 
and in the repsonse there is the attribute `"active": true`. Because the filter 
follows the rule to only pass inactive users, it sets the 
`"filter_is_anonymize_approval": false`.

deleted-user:

The step "Get user-data from the Jira user REST-API" queries the user's data from
`GET /rest/api/2/user?username=deleted-user`. 
It returns HTTP status-code `404 Not Found`, and in the response there is
the error-message
`"errorMessages": ["The user named 'deleted-user' does not exist"]`. Because the filter follows the rule to only pass
existent users, it sets the `"filter_is_anonymize_approval": false`.
...
Users `user1pre84` and `user1post84` are no reporter in any issue, nor assigned
to any user-field.
User `user2pre84` is assignee of issue EP-1, and `user2post84` is assignee of EP-2.
...
The output is:
    ...
    2020-12-27 20:51:45,958:INFO:read_user_names_from_infile():   The user-names are: ['user1pre84', 'user2pre84', 'user1post84', 'user2post84']
    ...
    2020-12-27 20:51:46,132:INFO:filter_users(): Remaining users to be anonymized: ['user1pre84', 'user2pre84', 'user1post84', 'user2post84']
    ...
    2020-12-27 20:51:46,147:INFO:run_user_anonymization(): for user (name/key) user1pre84/user1pre84
    2020-12-27 20:51:46,282:INFO:run_user_anonymization(): for user (name/key) user2pre84/user2pre84
    2020-12-27 20:51:48,380:INFO:wait_until_anonymization_is_finished_or_timedout(): for user user2pre84
    2020-12-27 20:51:48,395:INFO:run_user_anonymization(): for user (name/key) user1post84/JIRAUSER10200
    2020-12-27 20:51:48,427:INFO:run_user_anonymization(): for user (name/key) user2post84/JIRAUSER10201
    2020-12-27 20:51:50,453:INFO:wait_until_anonymization_is_finished_or_timedout(): for user user2post84
    Anonymizing Result:
      Users in infile:   4
      Skipped users:     0
      Deleted users:     2
...
for the users to assess the result of `filter_is_anonymize_approval` and what 
finally happened to the users.

user1pre84, user1post84:

The step "Get user-data from the Jira user REST-API" queries the user's data from
`GET /rest/api/2/user?username=user1pre84`. It returns HTTP status-code `200 OK`, 
and in the repsonse there is the attribute `"active": false`. The validations-API 
returned no error. Because the filter follows the rule to only pass
...
Jira does only allow deleting a user, if it is not in any user-field nor a 
comment-author. Why shall we keep and anonymize a user which never had left any 
traces in Jira? If the anonymizer fails deleting, it proceeds with anonymization.

user2pre84, user2post84:

Similar the previous user, but they are assignees in 1 issue each. 
The fact this user is an assignee is no validation error as this will not prevent
the anonymization. But deletion failed, so they were anonymized.

`user2pre84` was assignee of issue EP-1, and `user5post84` was assignees of EP-2.
...
Tells the Anonymizer which functions are available in addition.
...
Needs the add-on
[jira-anonymizinghelper](https://bitbucket.org/jheger/jira-anonymizinghelper/src/master/),
it provides `GET /rest/anonhelper/latest/applicationuser`.

During anonymization, the Jira REST-API does not provide information about the 
new user-name, -key, or the display-name (except in the atlassian-jira.log).
...
The anonymization described so far changes a user-name to the format "username12345",
the user-key to "JIRAUSER12345", and the display-name to the format "user-a2b4e".
We can see, some ID (12345) is the base to form the user-name and -key.

If we had this ID, we could predict the new user-name or -key, and 
query the user REST API again after the anonymization to get the new display-name.
...
- queries the anonymized user by the predicted user-name after anonymization,
- records the anonymized user-name, -key, and display-name in the reports.

### Example 4

We installed the jira-anonhelper-1.0.0.jar downloaded form 
[jira-anonymizinghelper](https://bitbucket.org/jheger/jira-anonymizinghelper/src/master/).

Similar to Example 3: We'll use the following config-file `my-config.cfg`, but
add `features = do_report_anonymized_user_data`:
    ...
    regular_delay = 2
    # Enable the feature:
    features = do_report_anonymized_user_data
...
The output is:
    ...
    2020-12-27 22:10:30,471:INFO:read_user_names_from_infile(): usernames.txt
    2020-12-27 22:10:30,477:INFO:read_user_names_from_infile():   The user-names are: ['user1pre84', 'user2pre84', 'user1post84', 'user2post84']
    2020-12-27 22:10:30,477:INFO:get_users_data_from_rest(): for 4 users
    2020-12-27 22:10:30,517:INFO:get_validation_data_from_rest(): 
    2020-12-27 22:10:30,620:INFO:get_applicationuser_data_from_rest(): GET /rest/anonhelper/latest/applicationuser
    2020-12-27 22:10:30,663:INFO:filter_users(): by existence and validation-data
    2020-12-27 22:10:30,663:INFO:filter_users(): Remaining users to be anonymized: ['user1pre84', 'user2pre84', 'user1post84', 'user2post84']
    2020-12-27 22:10:30,663:INFO:is_any_anonymization_running(): ?
    2020-12-27 22:10:30,663:INFO:get_anonymization_progress(): Checking if any anonymization is running
    2020-12-27 22:10:30,676:INFO:is_any_anonymization_running(): No
    2020-12-27 22:10:30,676:INFO:run_user_anonymization(): Going to delete or anonymize 4 users
    2020-12-27 22:10:30,676:INFO:run_user_anonymization(): for user (name/key) user1pre84/user1pre84
    2020-12-27 22:10:30,718:INFO:run_user_anonymization(): for user (name/key) user2pre84/user2pre84
    2020-12-27 22:10:32,790:INFO:wait_until_anonymization_is_finished_or_timedout(): for user user2pre84
    2020-12-27 22:10:32,803:INFO:run_user_anonymization(): for user (name/key) user1post84/JIRAUSER10200
    2020-12-27 22:10:32,832:INFO:run_user_anonymization(): for user (name/key) user2post84/JIRAUSER10201
    2020-12-27 22:10:34,859:INFO:wait_until_anonymization_is_finished_or_timedout(): for user user2post84
        ...
        "users": [
            {
                ...
                "filter_is_anonymize_approval": true,
                "filter_error_message": "",
                "time_start": "2020-12-27T22:10:30",
                "time_finish": null,
                "time_duration": null,
            ...
            },
            {
                ...
                "filter_error_message": "",
                "time_start": "2020-12-27T22:10:30",
                "time_finish": "2020-12-27T22:10:31",
                "time_duration": "00:01",
            ...
            },
            {
                ...
                "filter_is_anonymize_approval": true,
                "filter_error_message": "",
                "time_start": "2020-12-27T22:10:32",
                "time_finish": null,
                "time_duration": null,
                "anonymized_user_name": "",
                "anonymized_user_key": "",
            ...
            },
            {
                ...
                "filter_error_message": "",
                "time_start": "2020-12-27T22:10:32",
                "time_finish": "2020-12-27T22:10:32",
                "time_duration": "00:01",
...
The `anonymization_report.csv` looks like the following screenshot:

![](doc/images/example_4.png)
...
are expected in the language-setting of the executing admin.
But they're sometimes in a different language. 
Is this different language the one of the user to be anonymized, 
or the Jira-system-default-language? Or other?
